...
function formatClientSideAnswers(answers: any) {
  ...
  function handleSubmitDraft({answers, formId, setAutosavedQuestionId = undefined}: {answers: any, formId: number, setAutosavedQuestionId?: (id: number | null) => void}) {
        ...
        const url = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/forms/${formId}/submit`;
    ...
    return (<>
            ...
            <div>
                {questions.map((question: any, index: number) => (
                    <div key={index}>
                        <QuestionWrapper questionData={question} position={index + 1} value={answers[question.id]?.value || null} handleChange={handleAnswerChange} autosavedQuestionId={autosavedQuestionId} handleAutosave={handleAutosave} />
                    </div>
                ))}
            </div>
                <Button className="cursor-pointer" onClick={() => mutate({answers, formId: id})}><Spinner className={isPending ? "block w-6 h-6" : "hidden"} />{isPending ? "Submitting..." : "Submit"}</Button>
                </>
    )
}
